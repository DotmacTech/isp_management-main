...
These endpoints implement the authentication workflow as defined in the
...
# Import schemas directly from the parent module to avoid circular imports
from modules.auth.schemas import (
    ...
    MessageResponse, AuditLogList, TokenData
)

# Import services directly 
from modules.auth.services import AuthService

# Import dependencies from the root auth module, not from api
from modules.auth.dependencies import (
    get_current_user, get_current_active_user, 
    get_admin_user, get_staff_user, RateLimiter, get_db
)
<<<<<<< HEAD
=======
from backend_core.email_service import EmailService
from backend_core.audit_log import AuditLogService
from modules.auth.api.dependencies import RateLimiter


# Creating a factory function that returns an instance of RateLimiter
# This function is called by FastAPI's Depends to inject the RateLimiter dependency
def get_rate_limiter():
    # Return a new RateLimiter instance with a maximum of 5 requests in 3600 seconds (1 hour)
    return RateLimiter(max_requests=5, period=3600)

# Use FastAPI's Depends to inject the RateLimiter instance into your endpoint function
# The get_rate_limiter function is called automatically when the endpoint is accessed
...
async def register(
    ...
    rate_limiter: RateLimiter = Depends(RateLimiter(max_requests=5, period=3600))
):
    """Register a new user."""
    # Create the user
...
async def verify_mfa_login(
    ...
    request: Request,
    db: Session = Depends(get_db),
    rate_limiter: RateLimiter = Depends(RateLimiter(max_requests=5, period=300))
):
...
async def request_password_reset(
    ...
    background_tasks: BackgroundTasks,
    db: Session = Depends(get_db),
    rate_limiter: RateLimiter = Depends(RateLimiter(max_requests=3, period=3600))
):
